"""svf_linz_regression.py

Linz-style SQ4 regression analysis on SVF features against MMSE.

Replicates the methodology of Linz et al. (2017) — predict MMSE from
SVF-derived features using Support Vector Regression and Ridge
Regression with 5-fold cross-validation, report MAE with 95% bootstrap
CI — adapted to this thesis's Swedish synthetic dataset.

Pre-registered methodological decisions (do not change post-hoc):

1. **SVF cluster threshold pinned at 0.45** (Phase 3 calibrated value;
   the unique threshold at which both ``mean_cluster_size`` and
   ``switch_count`` cross p<0.001 in between-group Kruskal–Wallis).
   This script consumes Phase 3's scorer outputs directly; no
   re-calibration.

2. **Feature selection rule: |Spearman r| > 0.3 with MMSE.** Mirrors
   Linz et al. (2017) Table III, applied to *our* correlation table.
   Compute Spearman correlations between each Linz feature (WC, MCS,
   NOS, MWF, SD) and MMSE; features with |r| < 0.3 are excluded from
   the regression. The rule is fixed a priori; which features survive
   is determined by the data.

3. **Bootstrap MAE: error-based, 1000 iterations.** Run 5-fold CV,
   collect held-out predictions for all participants. Bootstrap-
   resample the resulting (predicted, actual) pairs 1000× with
   replacement; report mean MAE plus the 2.5th and 97.5th percentiles
   as the 95% CI. More rigorous than Linz's CV-fold-spread CI on small
   N.
...
- linz_confusion_matrix.csv       2×2 confusion at MMSE=24, stacked per model
- linz_kappa.csv                  Cohen's κ at MMSE=24 per model
- svf_predicted_vs_actual.png     2-panel scatter, diagnosis-coloured
...
LINZ_FEATURES: dict[str, str] = {
    # Linz abbreviation → column name in svf_results_with_mmse.csv
    "WC": "total_words",
    "MCS": "mean_cluster_size",
    "NOS": "switch_count",
    "MWF": "mean_word_frequency",
    "SD": "pairwise_similarity_mean",
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Linz-style SVF→MMSE regression (SQ4)."
    )
...
def load_and_validate(path: Path) -> pd.DataFrame:
    """Read the input CSV and assert the expected columns are present.

    Drops rows with NaN MMSE (with a warning); raises if more than 10%
    of rows would be dropped.
    """
    if not path.is_file():
...
def correlation_table(df: pd.DataFrame) -> pd.DataFrame:
    """Spearman r, p, and the |r|>0.3 pass flag for each Linz feature."""
    rows = []
    n = len(df)
...
def cv_predict(
    ...
    """Run StratifiedKFold by diagnosis, return held-out predictions.

    Inner GridSearchCV picks hyperparameters per fold. StandardScaler is
    fitted inside each pipeline so feature scaling does not leak across
    folds.
    """
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer.split(X, diagnosis)):
        ...
        # Inner CV folds cannot exceed train-set size; cap at 3 for tiny fixtures.
...
def bootstrap_mae(
    ...
    """Bootstrap MAE: mean and 2.5/97.5 percentiles over N resamples."""
...
def write_outputs(
    ...
    paths["correlation"] = output_dir / "linz_correlation_table.csv"
    corr_table.to_csv(paths["correlation"], index=False)

    paths["mae"] = output_dir / "linz_regression_mae.csv"
    pd.DataFrame([
        {
            "model": r["label"],
            "mae_mean": r["mae_mean"],
    ...
    paths["confusion"] = output_dir / "linz_confusion_matrix.csv"
    pd.DataFrame(cm_rows).to_csv(paths["confusion"], index=False)

    paths["kappa"] = output_dir / "linz_kappa.csv"
    pd.DataFrame([
    ...
    fig.suptitle("SVF → MMSE: predicted vs. actual (Linz-style regression)")
    fig.tight_layout()
...
def planned_summary() -> str:
    return (
        ...
        f"  5 Linz features (WC, MCS, NOS, MWF, SD)\n"
